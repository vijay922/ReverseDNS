...
parser.add_argument("-il", dest="file_input", help="eg: ip_list.txt", metavar="File Input")
parser.add_argument("-ir", dest="ip_ranges", help="eg: 192.168.0.1-192.168.2.0", metavar="IP Range")
parser.add_argument("-is", dest="subnet", help="eg: 192.168.0.1/24", metavar="Subnet")
parser.add_argument("-f", dest="save_file", help="write output to a file", metavar="Output file")
...
def resolve_ptr(ip):
    ...
    match_found = re.search('name pointer', p.decode())
    match_not_found = re.search('not found', p.decode())
    connection_timed_out = re.search('connection timed out', p.decode())
    global found_count, not_found_count, connection_timed_out_count
    if match_found:
        output = "%s  %s" % (ip, p.strip(b'\n').split(b' ')[-1].decode())
        print(output)
        found_count += 1
        if save_flag:
            f.write(output + "\n")
