...
func runDnsx(cidr string, outputFile *os.File) {
	cmd := exec.Command("sh", "-c", fmt.Sprintf("echo %s | dnsx -silent -resp-only -ptr", cidr))
	out, err := cmd.Output()
	if err != nil {
		fmt.Printf("Error running dnsx: %v\n", err)
		return
	}
